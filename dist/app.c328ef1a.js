...
function line(context, coords, _ref) {
  var color = _ref.color;
  // start drawing
  context.beginPath(); // line styles
  ...
  context.stroke(); // stop drawing

  context.closePath();
}

function circle(context, _ref2, color) {
...
var VIEW_WIDTH = DPI_WIDTH; // Function for init schedule

function chart(root, data) {
  // draw in 2d
  var canvas = root.querySelector('canvas');
  var context = canvas.getContext('2d');
  var ttp = (0, _tooltip.tooltip)(root.querySelector('[data-el="tooltip"]'));
  console.log(ttp);
  var reqAnimFrame; // canvas sizes
  ...
  canvas.height = DPI_HEIGHT; // Handle changes only if x changed
  ...
  function paint() {
    ...
    var yRation = VIEW_HEIGHT / (yMax - yMin);
    var xRation = VIEW_WIDTH / (data.columns[0].length - 2); // Columns for axis Y
    ...
    yData.map(toCoords(xRation, yRation)).forEach(function (coords, index) {
      var color = data.colors[yData[index][0]];
      (0, _utils.line)(context, coords, {
        color: color
      });
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          if ((0, _utils.isOver)(proxy.mouse, x, coords.length, DPI_WIDTH)) {
            (0, _utils.circle)(context, [x, y], color);
            break;
          }
        }
      } catch (err) {
  ...
  function yAxis(yMin, yMax) {
    ...
    for (var i = 1; i <= ROWS_COUNT; i++) {
      ...
      context.fillText(text.toString(), 5, y + PADDING - 10); // moving without connect
  ...
  function xAxis(xData, yData, xRation) {
    var labelsCount = 6; // step between labels

    var step = Math.round(xData.length / labelsCount);
    context.beginPath();
...
function toCoords(xRation, yRation) {
  return function (col) {
    return col.map(function (y, index) {
      return [Math.floor((index - 1) * xRation), Math.floor(DPI_HEIGHT - PADDING - y * yRation)];
    }).filter(function (_, i) {
      return i !== 0;
    });
  };
}
},{"./utils":"utils.js","./tooltip":"tooltip.js"}],"app.js":[function(require,module,exports) {
"use strict";
...
tgChart.init();
},{"./styles.scss":"styles.scss","./data":"data.js","./chart":"chart.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
